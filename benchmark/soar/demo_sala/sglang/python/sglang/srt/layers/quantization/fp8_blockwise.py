...
def _quantize_activation_fp8_blockwise(
    ...
    # Quantize activations — keep scale in x.dtype (BF16) to avoid float32 upcast
    # which would allocate a ~3.9 GB float32 intermediate for large prefill batches.
    scale_expanded = scales_row.to(x.dtype).unsqueeze(2)  # (M, K/128, 1) BF16
    x_fp8 = (x_blocks / scale_expanded).clamp(-FP8_MAX, FP8_MAX).to(
        torch.float8_e4m3fn
    ).reshape(M, K)
