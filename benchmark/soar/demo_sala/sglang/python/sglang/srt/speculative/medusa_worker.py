...
class MedusaWorker:
    ...
    def _forward_verify_k1(self, batch: ScheduleBatch) -> GenerationBatchResult:
        ...
        #
        # When accept_length >= 1, _free_cache's assign_req_to_token_pool call
        # writes (accept_length + 1) positions to req_to_token but out_cache_loc
        # only contains accept_length valid page-slot indices.  The extra read is
        # out-of-bounds and returns non-zero garbage from adjacent GPU memory.
        # This garbage value is stored at req_to_token[req_pool_idx, bonus_pos]
        # where bonus_pos = seq_lens_old + accept_length = new_seq_lens - 1.
        #
        # cache_finished_req's zero-filter (CHANGE_0159) catches sentinel slot 0
        # but passes non-zero garbage through, causing a spurious KV-slot free
        # that inflates available_size and eventually crashes the server.
        #
        # Fix: explicitly zero req_to_token at the bonus position so CHANGE_0159
        # can mask it out.  The bonus token's KV was never computed (the forward
        # pass only processes the draft tokens), so zeroing is semantically
        # correct — the bonus KV slot remains unallocated for this step.
        accept_lens_cpu = spec_info.accept_length.cpu().tolist()
        for _i, _req in enumerate(batch.reqs):
            if accept_lens_cpu[_i] >= 1:
                # bonus_pos = seq_lens_old + accept_length = new_seq_lens - 1
