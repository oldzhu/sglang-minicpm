...
**参考硬件**: SM120 RTX PRO 6000 — 296 TFLOPS FP8, 148 TFLOPS BF16, 593 TFLOPS FP4, 1398 GB/s
...
- 关键瓶颈：compute-bound 的 prefill 浪费了大量 SM120 FP8 算力余量
...
|------|-------------------|---------|------|
| TFLOPS 利用率 | ~100-140 BF16 有效 | ~200-260 FP8 | 1.6-2.0× |
| Prefill GEMM 时间 | 基准 | 减少 25-35% | ↑ |
| 端到端 S1 | 121.71s | 估计 ~95-110s | ↑ |
| 端到端 S8 | 44.09s | 估计 ~34-40s | ↑ |
...
|------|-------------------|---------|------|
| TFLOPS 利用率 | ~100-140 BF16 有效 | ~220-280 FP8 | 1.8-2.2× |
| Prefill GEMM 时间 | 基准 | 减少 30-45% | ↑ |
| 权重内存 | ~4.5GB W4 | ~4.5GB W4（不变！） | 相同 |
| 端到端 S1 | 121.71s | 估计 ~90-105s | ↑ |
| 端到端 S8 | 44.09s | 估计 ~32-38s | ↑ |
...
| **Prefill 加速** | GEMM 约 1.6-2.0× | GEMM 约 1.8-2.2× |
...
**Prefill 阶段**（长提示词时主导）：
- 大批量/序列长度时受计算限制
- 选项B：GEMM 组件（占 85% 时间）约 1.6-2.0× 加速 → 整体 prefill 减少 30-45%
- 选项C：GEMM 约 1.8-2.2× 加速 → 整体 prefill 减少 35-50%
- Δ（C vs B）：prefill 选项C好 5-10%
...
- 选项B：prefill 减少 35-45%
- 选项C：prefill 减少 40-50%
...
- 选项B：峰值收益，prefill 减少 40-50%
- 选项C：prefill 减少 45-55%
- 两个选项在 S∞ 都表现优秀
